const { User_Login_Information, User_Role, Field_Executive } = require('../../Configuration Files/Sequelize/Database_Synchronization'),
...
module.exports = ((app) => {
    ...
    app.post('/userSignUp', (req, res, next) => {
        ...
        const dbResponse = User_Login_Information.findOne({
            ...
            .catch((error) => {
                console.log('Error occur where the user is trying to register..... \n \n' + error)
        ...
        dbResponse.then((response) => {
            ...
            .catch((error) => {
                console.log('Error occur where the user is trying to register in the Promise Resolving..... \n \n' + error)
                req.session.SignUp_Error = {
                    type: 'Danger',
    ...
    app.post('/userSignUp', async (req, res) => {
        ...
        hashPassword = bcrypt.hashSync(req.body.registering_password, salt_ForBcrypt)
                        ...
                        */
                        ...
                        ejs.renderFile('./views/Web Appendage Pages/email_template.ejs', {
                            ...
                            if (err) {
                                ...
                                const emailStatus = Nodemailer.sendEmail_to_ConfirmEmail(response.dataValues.login_email, emailHTMLFILE, "Confirm your Email")
                                emailStatus.then((status) => {
                                    if (status) {
                                        /**
                                        ...
                                        User_Login_Information.update({
                                            ...
                                            .then(res => console.log('JWT is Updated for User UUID' + req.body.UUID))
                                            .catch(err => console.log(`Can not Update JWT for user UUID (${req.body.UUID}) with error: ` + err))
                                        res.status(200).send(
                                            {
                                    ...
                                    .catch((error) => {
                                        if (error) {
                                            res.status(404).send({ message: 'Sorry, The System is Busy. Please Wait ' })
                                        }
                                    })
                            }
                        })
