var express = require('express'),
  ...
  { Login_Page, Web_Content, SignUp_Page } = require('../../Configuration Files/Sequelize/Database_Synchronization')
...
router.get('/', isUser_Not_Login, function (req, res, next) {
  ...
  let dbResponse = Web_Content.findOne({
    attributes:
    {
      exclude: ['paused', 'deleted', 'sa_id', 'createdAt',
        'updateTimestamp', 'web_content_id', 'web_content_uuid']
    },
    limit: 1,
    raw: true,
    where:
...
router.get('/signup', isUser_Not_Login, function (req, res, next) {
  ...
  dbResponse.then((dbData) => {
    ...
    if (req.session.SignUp_Error) {
      console.log(req.session.SignUp_Error.type + req.session.SignUp_Error.message)
      res.status(200).render('Web Pages/signup',
        { message: req.flash(req.session.SignUp_Error.type, req.session.SignUp_Error.message), dbData })
